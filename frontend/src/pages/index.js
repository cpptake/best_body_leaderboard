...
import { evaluateImages, getBaselineImageUrl } from '../lib/api';
...
export default function Home() {
  ...
  // コンポーネントマウント時にベースライン画像のURLを取得
  useEffect(() => {
    const fetchBaselineImage = async () => {
      try {
        setBaselineLoading(true);
        const url = await getBaselineImageUrl();
        setBaselineImageUrl(url);
      } catch (err) {
        console.error('ベースライン画像の取得に失敗しました:', err);
        setError('ベースライン画像の読み込みに失敗しました');
      } finally {
